<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dianyingxinxi','购票') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="dianyinggoupiaoCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'购票':'' }}</el-button>
                <el-button v-if="isAuth('dianyingxinxi','购票') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="dianyinggoupiaoCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'购票':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dianyingxinxi','购票') && contents.tableBtnIcon == 0" type="success" size="mini" @click="dianyinggoupiaoCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'购票':'' }}</el-button>
    ...
    <dianyinggoupiao-cross-add-or-update v-if="dianyinggoupiaoCrossAddOrUpdateFlag" :parent="this" ref="dianyinggoupiaoCrossaddOrUpdate"></dianyinggoupiao-cross-add-or-update>
...
import dianyinggoupiaoCrossAddOrUpdate from "../dianyinggoupiao/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      dianyinggoupiaoCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    dianyinggoupiaoCrossAddOrUpdate,
  },
  methods: {
    ...
    dianyinggoupiaoCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.dianyinggoupiaoCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','dianyingxinxi');
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.dianyinggoupiaoCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.dianyinggoupiaoCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
