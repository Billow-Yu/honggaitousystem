<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.hunlianketang" label="婚恋课堂" prop="hunlianketang">
          <file-upload
          tip="点击上传婚恋课堂"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.hunlianketang?ruleForm.hunlianketang:''"
          @change="hunlianketangUploadChange"
          ></file-upload>
        </el-form-item> 
        <div v-else>
          <el-form-item v-if="ruleForm.hunlianketang" label="婚恋课堂" prop="hunlianketang">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.hunlianketang)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="相亲" prop="xiangqin">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.xiangqin" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.xiangqin" label="相亲" prop="xiangqin">
                    <span v-html="ruleForm.xiangqin"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.neirong = true;
          ...
          if(o=='hunlianketang'){
            this.ruleForm.hunlianketang = obj[o];
	    this.ro.hunlianketang = true;
            continue;
          }
          if(o=='xiangqin'){
            this.ruleForm.xiangqin = obj[o];
	    this.ro.xiangqin = true;
            continue;
          }
        }
      }
	...
	this.ruleForm.xiangqin = this.ruleForm.xiangqin.replace(reg,'../../../honggaitousystem/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.hunlianketang!=null) {
		this.ruleForm.hunlianketang = this.ruleForm.hunlianketang.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `hunlianketang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.hunlianketangCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    hunlianketangUploadChange(fileUrls) {
	this.ruleForm.hunlianketang = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
